...
def main():
    ...
    SignalforCylinder = None

    def callback(data):
		global SignalforCylinder
		rospy.loginfo(data.data)
		SignalforCylinder = data.data
		modclient.setOutput(8,False)
		modclient.setOutput(9,False)
		if SignalforCylinder == True:    			
			modclient.setOutput(8,True)
			rospy.signal_shutdown("End")

		if SignalforCylinder == False:
			modclient.setOutput(9,True)
			rospy.signal_shutdown("End")
	...
	if x == 0:
	   modclient.setOutput(8,1) #boolean
	
	if x > 0:
	   modclient.setOutput(1,1) #boolean
	   modclient.setOutput(2,0) #boolean
	   modclient.setOutput(3,x) #linear
	if x == 0:
	   modclient.setOutput(1,0) #boolean
	   modclient.setOutput(2,0) #boolean
	   modclient.setOutput(3,0) #linear

	if z > 0:
	   modclient.setOutput(4,1) #boolean
	   modclient.setOutput(5,0) #boolean
	   modclient.setOutput(6,z) #angular

	if z == 0:
	   modclient.setOutput(4,0) #boolean
	   modclient.setOutput(5,0) #boolean
	...
	if x < 0:
	   x = x*-1
	   modclient.setOutput(1,0) #boolean
	   modclient.setOutput(2,1) #boolean
	   modclient.setOutput(3,x) #linear

	if z < 0:
	   z = z*-1
	   modclient.setOutput(4,0) #boolean
	   modclient.setOutput(5,1) #boolean
	   modclient.setOutput(6,z) #angular

    

    sub = rospy.Subscriber("/cmd_vel",Twist,showUpdatedRegisters,queue_size=500)
    sub2 = rospy.Subscriber('ea_control',Bool,callback,queue_size=500)
